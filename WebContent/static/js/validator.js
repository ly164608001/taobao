...
function validateForm(formid){
    ...
    $(selectForm + ' input[type=text]').each(function(){
        ...
        var label = $inputObj.attr('label');
        if(label == undefined){
            label = '';
        }

        if(validateType != undefined){
            var validJson =  eval("(" + validateType + ")");
            var errMsg = '';
            //非空验证
            if(validJson.required && isNull(value)){
                return validFail(label + '不可为空',this);
            } 
            //最小值验证
            var minLen = validJson.minLen;
            if(minLen != undefined && !isMinLimit(value ,minLen) ){
                return validFail(label + '最少输入' + minLen + '字符',this);
            } 
            //最大值验证
            var maxLen = validJson.maxLen;
            if(maxLen != undefined && !isMaxLimit(value ,maxLen) ){
                return validFail(label + '最多输入' + maxLen + '字符',this);
            } 
            //验证数据格式
            ...
            if(type != undefined){
                ...
                if(type == 'email'  && !isEmail(value)){
                    errMsg = label + '非正确的邮箱格式';
                }
                //数字验证
                else if(type == 'num'  && !isNum(value)){
                    errMsg = label + '非数字';
                }
                //13手机号码验证
                else if(type == 'mobile' && !isMobilePhone(value)){
                    errMsg =  label + '非正确的手机号码';
                }
                //固话验证
                else if(type == 'tel' && !isTelephone(value)){
                    errMsg = label + '非正确的固话号码';
                }
                //QQ验证
                else if(type == 'QQ' && !isQQ(value)){
                    errMsg = label + '非正确的QQ号码';
                }
                //身份证验证
                else if(type == 'idCard' && !isCard(value)){
                    errMsg = label + '非正确的身份证号码';
                }
                //IP地址验证
                else if(type == 'ip' && !isIP(value)){
                    errMsg = label + '非正确的IP地址';
                }

                if(errMsg != ''){
                    return validFail(errMsg,this);
                }
