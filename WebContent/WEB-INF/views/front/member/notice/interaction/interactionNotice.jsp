...
$(document).ready(function(){
	...
	$(".input-reply input[type='button']").click(function(){
		...
		var url = "${basePath}front/member/notice/interaction/appraiseRecordReplySave.json";
			...
			success : function(result) {
				...
				if (res.success) {
					indexOk =top.layer.open({
			           ...
			           yes:function(){
			        		top.layer.close(indexOk);
			        		window.location = "${basePath}front/member/notice/interaction/interactionNotice.htm";
		                	return;
			           }
			       });
				} else {
...
<form id="searchForm" action="${basePath }front/member/notice/interaction/interactionNotice.htm">
                ...
                <div class="MessageCenter-main">
                	<div class="MessageCenter-nav">
                        <div class="MessageCenter-nav-l">
                            <ul class="clearfix">
                                <li><a class="li-h5" href="javascript:void(0);">楼盘点评：(<span>${appraiseNum}</span>)</a></li>
                                <li><a onclick="window.location.href='${basePath}front/member/notice/interaction/stationLetter.htm'" >站内信：(<span>${totalNum}</span>)</a></li>
                            </ul>
                        </div>
                    ...
                    <div class="read-nav clearfix">
                    	<ul>
                        	<li id="all" onclick="window.location.href='${basePath}front/member/notice/interaction/interactionNotice.htm'"  <c:if test="${empty vo.model.status}">class="li-h6"</c:if>>全部</li>
                            <li id="read" onclick="window.location.href='${basePath}front/member/notice/interaction/interactionNotice.htm?model.status=1'" <c:if test="${vo.model.status==1}">class="li-h6"</c:if>>已读</li>
                            <li id="unread" onclick="window.location.href='${basePath}front/member/notice/interaction/interactionNotice.htm?model.status=-1'" <c:if test="${vo.model.status==-1}">class="li-h6"</c:if>>未读</li>
                        </ul>
                    </div>
                    <form id="readMark" action="${basePath}front/member/notice/interaction/appraiseMarkRead.htm" method="post">
                    ...
                    <div class="read-main">
                        ...
                        <div id="all" class="command-main">
                        	<ul>
                        		 <c:forEach var="appraise" items="${list}" varStatus="i">
		                            	<li class="clearfix">
		                                	<input name="noticeIds" type="checkbox" value="${appraise.id}" style="margin-top:50px" />
		                                    <div class="command-pics">
		                                    	<div class="command-pic"><img src="${appraise.house.housespic}" width="100" height="100" /></div>
		                                        <p>${appraise.house.name}</p>
		                                    </div>
		                                    <div class="command-box">
		                                    	<div class="command-box-head"></div>
		                                        <div class="command-content">
		                                        	<div class="command-text clearfix">
		                                            	<span>${appraise.house.name}：</span>
		                                                <div class="text-main">${appraise.content}</div>
													</div>
		                                            <div class="text-time">${appraise.appraisetime}</div>
                            		                <c:forEach var="reply" items="${appraise.replyList}" >
	                                                    <div class="reply-content">
	                                                    	<span>${reply.user.username}：</span>
			                                            	<div class="reply-text">${reply.content}</div> 
			                                            	<div class="text-time text-r">${reply.appraisetime}</div>
	                                            		</div>
                                            		</c:forEach>
		                                            ...
		                                            <div class="input-reply" >
		                                            	...
		                                            	<input id="id_${i.index}" name="id" type="hidden" value="${appraise.id}" />
		                                            	<input id="btn_${i.index}"  type="button" value="回复" />
		                                            </div>
		                                        </div>
		                                        <span class="sharp-pic"></span>
		                                        <c:choose>
		                                        <c:when test="${appraise.status == 1}">
