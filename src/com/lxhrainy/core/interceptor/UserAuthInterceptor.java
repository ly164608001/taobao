...
public class UserAuthInterceptor extends HandlerInterceptorAdapter {
	...
	private boolean hasMenuAuth(HttpServletRequest request){
		...
		if(!bMgrUrl && (requestPath.indexOf("admin/login.htm")!=-1||funcid.length()==0)){
			return true;
		} 
        ...
        //获取用户的菜单列表
        List<SysMenu> menuList = sysMenuService.findByUserIdWithType(userid, null);
	...
	public ModelAndView forword(HttpServletRequest request) {
		return new ModelAndView(new RedirectView("admin/login.htm"));
	}
